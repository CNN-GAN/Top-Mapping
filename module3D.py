...
def encoder3D(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope("ENCODER", reuse=reuse):
        ...
        net_h1 = Conv3dLayer(net_h0, act=None, shape=[3,3,3,args.voxel_filter,args.voxel_filter*2], stride=[1,2,2,1,1],\
        ...
        net_h2 = Conv3dLayer(net_h1, act=None, shape=[3,3,3,args.voxel_filter*2,args.voxel_filter*4], stride=[1,2,2,1,1],\
        ...
        net_h3 = Conv3dLayer(net_h2, act=None, shape=[3,3,3,args.voxel_filter*4,args.voxel_filter*8], stride=[1,2,2,2,1],\
...
def decoder3D(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope("DECODER", reuse=reuse):
        ...
        net_h0 = DenseLayer(net_in, n_units=args.img_filter*8*s16*s16*s16, W_init=w_init,
                            act = tf.identity, name='De/h0/lin')
        net_h0 = ReshapeLayer(net_h0, shape=[-1, s16, s16, s16,args.img_filter*8], name='De/h0/reshape')
        net_h0 = BatchNormLayer(net_h0, act=tf.nn.relu, is_train=is_train,
                                gamma_init=gamma_init, name='De/h0/batch_norm')

        net_h1 = DeConv3dLayer(net_h0, act=None, shape=[3,3,3,args.voxel_filter*4,args.voxel_filter*8], \
                               stride=[1,2,2,2,1], padding='SAME', \
                               W_init=w_init, b_init=b_init, name='De/h1/conv3d')
        net_h1 = BatchNormLayer(net_h1, act=lambda x: tl.act.lrelu(x, 0.2), \
                                is_train=is_train, gamma_init=gamma_init, name='De/h1/batch_norm')


        net_h2 = DeConv3dLayer(net_h1, act=None, shape=[3,3,3,args.voxel_filter*2,args.voxel_filter*4], \
                               stride=[1,2,2,2,1], padding='SAME', \
                               W_init=w_init, b_init=b_init, name='De/h2/conv3d')
        net_h2 = BatchNormLayer(net_h2, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='De/h2/batch_norm')


        net_h3 = DeConv3dLayer(net_h2, act=None, shape=[3,3,3,args.voxel_filter,args.voxel_filter*2], \
                               stride=[1,2,2,2,1], padding='SAME', \
                               W_init=w_init, b_init=b_init, name='De/h3/conv2d')
        net_h3 = BatchNormLayer(net_h3, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='De/h3/batch_norm')


        net_h4 = DeConv3dLayer(net_h3, act=None, shape=[3,3,3,1,args.voxel_filter], stride=[1,2,2,2,1],\
                               padding='SAME', W_init=w_init, b_init=b_init, name='De/h4/conv2d')
...
def discriminator_X(input_X, is_train=True, reuse=False):
    ...
    with tf.variable_scope("DISC_X", reuse=reuse):
        ...
        net_in = InputLayer(inputs, name='DX/in')
        net_h0 = Conv3dLayer(net_in, act=None, shape=[3,3,3,1,args.voxel_filter], stride=[1,2,2,2,1],\
                             padding='SAME', W_init=w_init, b_init=b_init, name='DX/h0/conv2d')
        net_h0 = BatchNormLayer(net_h0, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h0/batch_norm')

        net_h1 = Conv3dLayer(net_h0, act=None, shape=[3,3,3,args.voxel_filter,args.voxel_filter*2], \
                             stride=[1,2,2,1,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h1/conv2d')
        net_h1 = BatchNormLayer(net_h1, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h1/batch_norm')

        net_h2 = Conv3dLayer(net_h1, act=None, shape=[3,3,3,args.voxel_filter*2,args.voxel_filter*4],\
                             stride=[1,2,2,1,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h2/conv2d')
        net_h2 = BatchNormLayer(net_h2, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h2/batch_norm')

        net_h3 = Conv3dLayer(net_h2, act=None, shape=[3,3,3,args.voxel_filter*4,args.voxel_filter*8], \
                             stride=[1,2,2,2,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h3/conv2d')
        net_h3 = BatchNormLayer(net_h3, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h3/batch_norm')

        net_h4 = FlattenLayer(net_h3, name='DX/h4/flatten')
        net_h4 = DenseLayer(net_h4, n_units=args.code_dim, act=tf.identity,
...
def discriminator_J(input_X, input_Z, is_train=True, reuse=False):
    ...
    with tf.variable_scope("DISC_J", reuse=reuse):
        ...
        netX_h0 = Conv3dLayer(netX_in, act=None, shape=[3,3,3,1,args.voxel_filter], stride=[1,2,2,2,1],\
                             padding='SAME', W_init=w_init, b_init=b_init, name='DX/h0/conv2d')
        netX_h0 = BatchNormLayer(netX_h0, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h0/batch_norm')

        netX_h1 = Conv3dLayer(netX_h0, act=None, shape=[3,3,3,args.voxel_filter,args.voxel_filter*2], \
                             stride=[1,2,2,1,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h1/conv2d')
        netX_h1 = BatchNormLayer(netX_h1, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h1/batch_norm')

        netX_h2 = Conv3dLayer(netX_h1, act=None, shape=[3,3,3,args.voxel_filter*2,args.voxel_filter*4],\
                             stride=[1,2,2,1,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h2/conv2d')
        netX_h2 = BatchNormLayer(netX_h2, act=lambda x: tl.act.lrelu(x, 0.2),
                                is_train=is_train, gamma_init=gamma_init, name='DX/h2/batch_norm')

        netX_h3 = Conv3dLayer(netX_h2, act=None, shape=[3,3,3,args.voxel_filter*4,args.voxel_filter*8], \
                             stride=[1,2,2,2,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h3/conv2d')
