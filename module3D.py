...
def encoder(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope("ENCODER", reuse=reuse):
        ...
        net_h2 = Conv3d(net_h1, args.voxel_filter*4, (5, 5, 5), (2, 2, 2), act=None,
...
def decoder(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope("DECODER", reuse=reuse):
        ...
        net_h0 = DenseLayer(net_in, n_units=args.voxel_filter*8*s16*s16*s16, W_init=w_init,
                            act = tf.identity, name='De/h0/lin')
        net_h0 = ReshapeLayer(net_h0, shape=[-1, s16, s16, s16, args.voxel_filter*8], name='De/h0/reshape')

        net_h1 = DeConv3d(net_h0, args.voxel_filter*4, (5, 5, 5), out_size=(s8, s8, s8), strides=(2, 2, 2),
        ...
        net_h2 = DeConv3d(net_h1, args.voxel_filter*2, (5, 5, 5), out_size=(s4, s4, s4), strides=(2, 2, 2),
        ...
        net_h3 = DeConv3d(net_h2, args.voxel_filter*2, (5, 5, 5), out_size=(s2, s2, s2), strides=(2, 2, 2),
        ...
        net_h4 = DeConv3d(net_h3, args.voxel_filter, (5, 5, 5), out_size=(s2, s2, s2), strides=(2, 2, 2),
                          padding='SAME', batch_size=args.batch_size, act=None, W_init=w_init, name='De/h3/decon2d')
        net_h4 = BatchNormLayer(net_h4, act=tf.nn.relu, is_train=is_train,
                                gamma_init=gamma_init, name='De/h3/batch_norm')

        net_h4 = DeConv3d(net_h3, args.voxel_dim, (5, 5, 5), out_size=(s0, s0, s0), strides=(2, 2, 2),
...
def discriminator_J(input_X, input_Z, is_train=True, reuse=False):
    ...
    with tf.variable_scope("DISC_J", reuse=reuse):
        ...
        netX_h0 = Conv3dLayer(netX_in, act=None, shape=[3,3,3,1,args.voxel_filter], strides=[1,2,2,2,1],\
        ...
        netX_h1 = Conv3dLayer(netX_h0, act=None, shape=[3,3,3,args.voxel_filter,args.voxel_filter*2], \
                             strides=[1,2,2,1,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h1/conv2d')
        ...
        netX_h2 = Conv3dLayer(netX_h1, act=None, shape=[3,3,3,args.voxel_filter*2,args.voxel_filter*4],\
                             strides=[1,2,2,1,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h2/conv2d')
        ...
        netX_h3 = Conv3dLayer(netX_h2, act=None, shape=[3,3,3,args.voxel_filter*4,args.voxel_filter*8], \
                             strides=[1,2,2,2,1], padding='SAME', W_init=w_init, b_init=b_init, name='DX/h3/conv2d')
