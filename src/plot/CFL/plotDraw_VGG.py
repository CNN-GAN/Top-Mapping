...
def Plot_VGG(args):
    ...
    for route_id, route_name in enumerate(route_dir):
        for w_i in range(len(test_dir)):
            Trainvector_path = os.path.join(result_dir,  route_name+'_'+test_dir[w_i]+'_vt.npy')
            train_code = np.load(Trainvector_path)
            
            for w_j in range(w_i+1, len(test_dir)):
                
                file_name = route_name+'_'+test_dir[w_i]+'_'+test_dir[w_j]
                print('Load data file:{}'.format(test_dir[w_j])) 
                Testvector_path = os.path.join(result_dir, route_name+'_'+test_dir[w_j]+'_vt.npy')
